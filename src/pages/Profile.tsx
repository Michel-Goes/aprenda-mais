...
export default function Profile({ onSettingsClick, userName = 'Estudante', avatarUrl = null }: ProfileProps & { avatarUrl?: string | null }) {
  ...
  const allMedalsList = [
    ...
    { label: 'Matemático Brilhante', icon: Flame, color: 'bg-orange-100 text-orange-600' },
    { label: 'Colecionador', icon: Star, color: 'bg-purple-100 text-purple-600' },
    { label: 'Ofensiva Rei', icon: Flame, color: 'bg-red-100 text-red-600' },
    { label: 'Sabe Tudo', icon: Award, color: 'bg-teal-100 text-teal-600' },
    { label: 'Velocista', icon: Flame, color: 'bg-pink-100 text-pink-600' },
    { label: 'Veterano', icon: Award, color: 'bg-indigo-100 text-indigo-600' },
  ];
      ...
      <section>
        <div className="flex justify-between items-center mb-6">
          ...
          <button 
            ...
            className="text-primary font-bold text-sm hover:underline font-label p-2 -mr-2"
          >
            Ver tudo
          </button>
        </div>
        <div className="grid grid-cols-3 gap-4">
          {allMedalsList.slice(0, 3).map((medal, i) => (
            <motion.div
              key={i}
